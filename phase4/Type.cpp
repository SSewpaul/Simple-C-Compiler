...
bool Type::isPointer() const{
    return (_indirection > 0 && _declarator==SCALAR) || _declarator == ARRAY;
}
...
bool Type::isCompatibleWith(const Type& that) const{
    Type lhs = promote();
    Type rhs = that.promote();

    if (lhs.isNumeric() && rhs.isNumeric())
        return true;
    else if(lhs.isPointer() && rhs.isPointer()){
        if(lhs.specifier() == VOID || rhs.specifier() == VOID)
            return true;
        else if(lhs.specifier() == rhs.specifier() && lhs.indirection() == rhs.indirection())
        {
            return true;
        }
    }
