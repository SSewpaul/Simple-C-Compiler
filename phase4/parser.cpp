...
static Type primaryExpression(bool &lvalue)
{
	...
	else if (lookahead == STRING)
	{
		match(STRING);
		lvalue = false;
		return Type(CHAR, 0, 42);
	}
	else if (lookahead == NUM)
	{
		// char * ptr;
		long res = strtol(lexbuf.c_str(), NULL, 0);
		match(NUM);
		lvalue = false;
	...
	else if (lookahead == ID)
	{
		...
		if (lookahead == '(')
		{
			...
			if (lookahead != ')')
			{
				...
				res = checkFunction(id->type(), args);
				lvalue = false;
			}
...
static Type multiplicativeExpression(bool &lvalue)
{
	...
	while (1)
	{
		if (lookahead == '*')
		{
			...
			left = checkMulDivRem(left, right, "*");
			lvalue = false;
		}
		else if (lookahead == '/')
		{
...
static void assignment(bool &lvalue)
{
	...
	bool init_lvalue = lvalue;

	if (lookahead == '=')
	{
		match('=');
		Type right = expression(lvalue);
		checkAssignment(left, right, init_lvalue);
	}
}
